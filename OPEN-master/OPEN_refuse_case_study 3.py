...
input_data_path = os.path.join("Data", "RefuseCaseStudy.csv")
...
Emax_EV = 30  # maximum EV energy level (size of usable battery (kWh))
Emin_EV = 0  # minimum EV energy level 
P_max_EV = 50  # maximum EV charging power (power of charger for charging)
P_min_EV = -50  # minimum EV charging power (power of charger for exporting e.g. V2G)
...
# random EV arrival times between 16:00 and 18:00
ta_EVs = np.random.randint(int(16 / dt_ems), int(18 / dt_ems), N_EVs) - int(T0 / dt_ems)
# random EV departure times between 5am and 6am (add 24 for next day)
td_EVs = np.random.randint(int(29 / dt_ems), int(30 / dt_ems), N_EVs) - int(T0 / dt_ems)
